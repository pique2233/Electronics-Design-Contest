...
//        scanf("%d %d", &points[i].x, &points[i].y);
        points[0].x=g_ucaUsart2ReceiveBuffer[2]-44.5;
        points[0].y=g_ucaUsart2ReceiveBuffer[3]-45.5;
        
        points[1].x=g_ucaUsart2ReceiveBuffer[4]-44.5;
        points[1].y=g_ucaUsart2ReceiveBuffer[5]-45.5;
        
        points[2].x=g_ucaUsart2ReceiveBuffer[6]-44.5;
        points[2].y=g_ucaUsart2ReceiveBuffer[7]-45.5;
        
        points[3].x=g_ucaUsart2ReceiveBuffer[8]-44.5;
        points[3].y=g_ucaUsart2ReceiveBuffer[9]-45.5;
...
//    printf("Points in clockwise order:\n");
    for (int i = 3; i>=0; i--) {
         NEW_points[i].x=5.618 * points[i].x;   //500/89
         NEW_points[i].y=5.495 * points[i].y; //500/91
...
//    HAL_Delay(2000);
    ...
    HAL_Delay(6000);      
    slow_motion (NEW_points[3].x, NEW_points[2].x, NEW_points[3].y, NEW_points[2].y,50);
    //HAL_Delay(2000);
    slow_motion (NEW_points[2].x, NEW_points[1].x, NEW_points[2].y, NEW_points[1].y,50);
   // HAL_Delay(2000);
    slow_motion (NEW_points[1].x, NEW_points[0].x, NEW_points[1].y, NEW_points[0].y,50);
   // HAL_Delay(2000);
     slow_motion (NEW_points[0].x, NEW_points[3].x, NEW_points[0].y, NEW_points[3].y,50);
   // HAL_Delay(2000);
